...
def apply_column_mapping(df, column_map):
    ...
    Enhanced with currency parsing for price columns.
    """
    df_mapped = df.copy()
    ...
    for col in currency_columns:
        if col in df_mapped.columns:
            df_mapped[col] = parse_currency(df_mapped[col])
    ...
    if 'Total_Sales' not in df_mapped.columns:
        if 'Quantity' in df_mapped.columns and 'UnitPrice' in df_mapped.columns:
            try:
                df_mapped['Total_Sales'] = (
                    pd.to_numeric(df_mapped['Quantity'], errors='coerce') * 
                    pd.to_numeric(df_mapped['UnitPrice'], errors='coerce')
                )
            except Exception:
                pass
    
    # Ensure numeric columns are properly typed
    numeric_cols = ['Total_Sales', 'Quantity', 'UnitPrice']
